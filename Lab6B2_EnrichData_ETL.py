...
#---------------------------------------------------
print("JOB STARTED...")
car_sales     = spark.sql(f"SELECT * FROM {username}_sales.car_sales")
customer_data = spark.sql(f"SELECT * FROM {username}_marketing.customer_data")
car_installs  = spark.sql(f"SELECT * FROM {username}_sales.car_installs")
factory_data  = spark.sql(f"SELECT * FROM {username}_factory.experimental_motors")
geo_data      = spark.sql(f"SELECT postalcode as zip, latitude, longitude FROM {username}_marketing.geo_data_xref")
...
salesandcustomers_sql = f"SELECT customers.*, sales.sale_date, sales.saleprice, sales.model, sales.VIN \
                            FROM {username}_sales.car_sales sales JOIN {username}_marketing.customer_data customers \
...
#---------------------------------------------------
spark.sql(f"DROP TABLE IF EXISTS {username}_FACTORY.EXPERIMENTAL_MOTOR_ENRICHED")
tempTable.write.mode("overwrite").saveAsTable(username + '_factory.experimental_motor_enriched', format="parquet")
print(f"\tNEW ENRICHED TABLE CREATED: {username}_FACTORY.EXPERIMENTAL_MOTOR_ENRICHED")
tempTable.show(n=5)
