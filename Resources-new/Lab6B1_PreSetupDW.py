...
#---------------------------------------------------
### Update the username
username = "ENTER YOUR USER NAME HERE" ## Ex:- wuser1

appName = username + "-Pre-SetupDW"
...
#-----------------------------------------------------------------------------------
s3BucketName = "s3a://pse-workshop-kyndryl-bucket"         # <--- Update S3 bucket location
...
#---------------------------------------------------
print("JOB STARTED...")
spark.sql(f"DROP DATABASE IF EXISTS {username}_SALES CASCADE")
spark.sql(f"DROP DATABASE IF EXISTS {username}_FACTORY CASCADE")
spark.sql(f"DROP DATABASE IF EXISTS {username}_MARKETING CASCADE")
...
##                 CREATE DATABASES
##---------------------------------------------------
spark.sql(f"CREATE DATABASE {username}_SALES")
spark.sql(f"CREATE DATABASE {username}_FACTORY")
spark.sql(f"CREATE DATABASE {username}_MARKETING")
print("\tCREATE DATABASE(S) COMPLETED")
print("\n Databases created are shown below:-")
spark.sql(f"show databases like '{username}*' ").show()

#---------------------------------------------------
#               POPULATE TABLES
#---------------------------------------------------
car_sales.write.mode("overwrite").saveAsTable(username + '_SALES.CAR_SALES', format="parquet")
car_installs.write.mode("overwrite").saveAsTable(username+'_SALES.CAR_INSTALLS', format="parquet")
factory_data.write.mode("overwrite").saveAsTable(username + '_FACTORY.EXPERIMENTAL_MOTORS', format="parquet")
customer_data.write.mode("overwrite").saveAsTable(username + '_MARKETING.CUSTOMER_DATA', format="parquet")
geo_data.write.mode("overwrite").saveAsTable(username + '_MARKETING.GEO_DATA_XREF', format="parquet")
